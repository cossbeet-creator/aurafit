...
`;
  ...
  useEffect(() => {
    ...
    // 一時保存データのチェックと復元（日付ごとのキーを使用）
    const tempRecordsStr = localStorage.getItem(`fitrum_temp_exercise_records_${selectedDateStr}`);
  ...
  useEffect(() => {
    ...
    if (exerciseRecords.length > 0 && !isCompleted && !isEditingPast) {
      localStorage.setItem(`fitrum_temp_exercise_records_${selectedDateStr}`, JSON.stringify(exerciseRecords));
    } else if (isCompleted || isEditingPast) {
      // 完了済みまたは過去編集モードの場合は一時保存をクリーンアップ
      localStorage.removeItem(`fitrum_temp_exercise_records_${selectedDateStr}`);
    }
